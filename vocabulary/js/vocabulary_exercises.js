...
const counter = localStorage.getItem(`${localStorage.getItem("currentLanguage")}vocabulary`);
    ...
    if (confirmButton.textContent === "Confirm Answer") {
        ...
        if (doneQuestions === necessaryQuestions) {
            ...
            if(+counter < (selected + 1)*repetitions){
                localStorage.setItem(`${localStorage.getItem("currentLanguage")}vocabulary`, +counter + 1)
            }
        } else {
            setQuestion();
        }
