...
function setQuestion() {
    ...
    questionElements[0].textContent = currentLessonJson[currentQuestionNo].Rule;
    ...
    if (confirmButton.textContent === "Confirm Answer") {
        ...
        if (doneQuestions === necessaryQuestions) {
            ...
            if(+counter < (selected + 1)*repetitions){
                localStorage.setItem(`${localStorage.getItem("currentLanguage")}grammar`, +counter + 1)
            }
        } else {
            setQuestion();
        }
...
function showHint() {
    hintButton.textContent = localStorage.getItem(`${localStorage.getItem("currentLanguage")}:Grammar:${selected},${currentQuestionNo}`);
    if(localStorage.getItem(`${localStorage.getItem("currentLanguage")}:Grammar:${selected},${currentQuestionNo}`) === null || localStorage.getItem(`${localStorage.getItem("currentLanguage")}:Grammar:${selected},${currentQuestionNo}`) === "")
    {
