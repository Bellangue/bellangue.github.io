...
for (let i = 0; i < lessonNameData.length; ++i) {
    ...
    for (let j = 0; j < lessonJSONData[i].length; ++j) {
        ...
        inputLi.value = localStorage.getItem(`${localStorage.getItem("currentLanguage")}:Listening:${i},${j}`)
...
for (let i = 0; i < lessonNameData.length; ++i) {
    for (let j = 0; j < lessonJSONData[i].length; ++j) {
        ...
        affectedInput.addEventListener("change", function () {
            localStorage.setItem(`${localStorage.getItem("currentLanguage")}:Listening:${i},${j}`, affectedInput.value);
        });
    }
}