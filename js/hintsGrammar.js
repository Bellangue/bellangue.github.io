...
for (let i = 0; i < lessonNameData.length; ++i) {
    ...
    for (let j = 0; j < lessonJSONData[i].length; ++j) {
        ...
        hintli.innerHTML = `${j + 1}) ${lessonJSONData[i][j].Question} <br> ${lessonJSONData[i][j].Rule}`;
        ...
        inputLi.value = localStorage.getItem(`${localStorage.getItem("currentLanguage")}:Grammar:${i},${j}`)
...
for (let i = 0; i < lessonNameData.length; ++i) {
    for (let j = 0; j < lessonJSONData[i].length; ++j) {
        ...
        affectedInput.addEventListener("change", function () {
            localStorage.setItem(`${localStorage.getItem("currentLanguage")}:Grammar:${i},${j}`, affectedInput.value);
        });
    }
}